@extends('admin.app')
@section('title', __('dashboard.Edit Contact Type'))
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header border-transparent">
                    <h3 class="card-title">@lang('dashboard.Edit Contact Type') #{{ $contact_type->id }}</h3>
                ...
                <div class="card-body">
                    <form action="{{ route('admin.contact-types.update', $contact_type->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="name_ar">@lang('dashboard.name in arabic')</label>
                            <input type="text" class="form-control" name="name_ar" id="name_ar" 
                                   value="{{ old('name_ar', $contact_type->name_ar) }}" required>
                            @error('name_ar')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="name_en" id="name_en" 
                                   value="{{ old('name_en', $contact_type->name_en) }}" required>
                            @error('name_en')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="type" id="type" required>
                                <option value="support" {{ old('type', $contact_type->type) == 'support' ? 'selected' : '' }}>
                                    @lang('dashboard.support')
                                </option>
                                <option value="help" {{ old('type', $contact_type->type) == 'help' ? 'selected' : '' }}>
                                    @lang('dashboard.help')
                                </option>
                            </select>
                            @error('type')
