...
class UpdateAdTowerTest extends TestCase
{
    use RefreshDatabase;

    /** @test */
    public function it_updates_a_tower_ad_successfully()
    {
        // Create a user and log them in
        $user = User::factory()->create();
        $token = auth('api')->login($user);

        // Create a category for "Tower"
        $category = Category::factory()->create([
            'id' => 20,
            'adable' => "App\Models\Tower",
        ]);

        // Create a sample tower ad to update
