...
class AdRentController extends Controller
{
    public function index(Request $request)
    {
        $query = Ad::with(['category', 'user'])->where('for_rent', 1);
        ...
        $categories = Category::where('for_rent', 1)->get();

        $basicFilterWhenExport = ['for_rent' => 1];

        $routeName = 'admin.ads-rent.index';
        $routeName = 'admin.ads-rent.index';
