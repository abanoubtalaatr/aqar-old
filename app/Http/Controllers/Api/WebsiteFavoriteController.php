...
use App\Http\Resources\Api\WebsiteFavoriteResource;

class WebsiteFavoriteController extends Controller
{
    use GeneralTrait;
    ...
    public function myFavorites(Request $request)
    {
        ...
        if ($request->has('paginate') && $request->input('paginate') == 1) {
            $favorites_data = WebsiteFavoriteResource::collection($favorites)->response()->getData();
        } else {
            $favorites_data = WebsiteFavoriteResource::collection($favorites);
        }
    ...
    public function myAdFavorites(Request $request)
    {
        ...
        // Transform the favoritable (Ad) directly to SimpleAdResource
        $ads = $favorites->map(function ($favorite) {
        ...
        return $this->returnData('data', $ads_data, 'My Ad favorites retrieved successfully');
    }
    ...
    public function myOrderFavorites(Request $request)
{
    ...
    // Transform the favoritable (Order) directly to OrderResource
    $orders = $favorites->map(function ($favorite) {
        return $favorite->favoritable;
    });
    ...
    return $this->returnData('data', $orders_data, 'My Order favorites retrieved successfully');
}
